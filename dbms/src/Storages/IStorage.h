...
public:
    ...
    TableStructureLockHolder lockStructure(bool will_insert_data, const String & query_id)
    {
        TableStructureLockHolder result;
        if (will_insert_data)
            result.insert_structure_lock = insert_structure_lock->getLock(RWLockImpl::Read, query_id);
        else
            result.select_structure_lock = select_structure_lock->getLock(RWLockImpl::Read, query_id);
    ...
    void lockDataForAlter(TableStructureLockHolder & lock_holder, const String & query_id)
    {
        ...
        lock_holder.insert_structure_lock = insert_structure_lock->getLock(RWLockImpl::Write, query_id);
        if (is_dropped)
            throw Exception("Table is dropped", ErrorCodes::TABLE_IS_DROPPED);
    }
    ...
    void lockStructureForAlter(TableStructureLockHolder & lock_holder, const String & query_id)
    {
        ...
        if (!lock_holder.insert_structure_lock)
            lock_holder.insert_structure_lock = insert_structure_lock->getLock(RWLockImpl::Write, query_id);

        lock_holder.select_structure_lock = select_structure_lock->getLock(RWLockImpl::Write, query_id);
    ...
    TableStructureLockHolder lockExclusively(const String & query_id)
    {
        ...
        result.alter_intention_lock = alter_intention_lock->getLock(RWLockImpl::Write, query_id);
        result.insert_structure_lock = insert_structure_lock->getLock(RWLockImpl::Write, query_id);
        result.select_structure_lock = select_structure_lock->getLock(RWLockImpl::Write, query_id);
...
private:
    ...
    /** It is taken for read for the entire INSERT query and the entire merge of the parts (for MergeTree).
      ...
      */
    mutable RWLock insert_structure_lock = RWLockImpl::create();
    ...
    /** Lock for multiple columns and path to table. It is taken for write at RENAME, ALTER (for ALTER MODIFY for a while) and DROP.
      ...
      */
    mutable RWLock select_structure_lock = RWLockImpl::create();
};
